...
⏱ <b>Time Left:</b> <code>60 sec</code>""",
    ...
    "STREAMUP_API": (
        "String",
        "StreamUP API Key",
        "<i>Send your StreamUP API Key.</i> \n┖ <b>Time Left :</b> <code>60 sec</code>",
    ),
    "ANONFILES_API": (
        "String",
        "AnonFiles API Key",
...
┖ <b>PixelDrain Key</b> → <code>{pdtoken}</code>"""
        ...
        buttons.data_button("StreamUP API", f"userset {user_id} menu STREAMUP_API")
        buttons.data_button("Back", f"userset {user_id} back uphoster", "footer")
        buttons.data_button("Close", f"userset {user_id} close", "footer")
        btns = buttons.build_menu(1)

        if user_dict.get("STREAMUP_API", False):
            sutoken = user_dict["STREAMUP_API"]
        elif Config.STREAMUP_API:
            sutoken = Config.STREAMUP_API
        else:
            sutoken = "None"

        text = f"""⌬ <b>StreamUP Settings :</b>
┟ <b>Name</b> → {user_name}
┃
┖ <b>StreamUP API</b> → <code>{sutoken}</code>"""

    elif stype == "anonfiles":
        buttons.data_button("AnonFiles API", f"userset {user_id} menu ANONFILES_API")
        buttons.data_button("Back", f"userset {user_id} back uphoster", "footer")
        buttons.data_button("Close", f"userset {user_id} close", "footer")
        btns = buttons.build_menu(1)

        if user_dict.get("ANONFILES_API", False):
            aftoken = user_dict["ANONFILES_API"]
        elif Config.ANONFILES_API:
            aftoken = Config.ANONFILES_API
        else:
            aftoken = "None"

        text = f"""⌬ <b>AnonFiles Settings :</b>
┟ <b>Name</b> → {user_name}
┃
